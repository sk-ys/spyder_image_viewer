...
class ImageViewerWidget(PluginMainWidget):
    ...
    def _event_pos(event):
        """Return mouse position in viewport coordinates for Qt5/Qt6."""
        if hasattr(event, "position"):
            return event.position().toPoint()
        return event.pos()

    def _start_drag_pan(self, pos) -> bool:
        """Start left-drag panning when an image is visible."""
        if self.original_pixmap is None:
            return False
    ...
    def _apply_drag_pan(self, pos) -> None:
        """Pan the viewport by dragging the mouse."""
        if not self._is_drag_panning or self._drag_last_pos is None:
            return

        delta_x = int(pos.x() - self._drag_last_pos.x())
        delta_y = int(pos.y() - self._drag_last_pos.y())
        self._drag_last_pos = pos
    ...
    def _stop_drag_pan(self) -> None:
        """Stop drag panning and restore cursor state."""
        self._is_drag_panning = False
        self._drag_last_pos = None
    ...
    def eventFilter(self, watched, event):
        ...
        if watched is self.scroll_area.viewport() and event.type() == QEvent.MouseMove:
            if self._is_drag_panning:
                self._apply_drag_pan(self._event_pos(event))
                event.accept()
                return True
            self._update_hover_tooltip(event)

        if watched is self.scroll_area.viewport() and event.type() == QEvent.MouseButtonRelease:
            if event.button() == Qt.LeftButton and self._is_drag_panning:
                self._stop_drag_pan()
                event.accept()
                return True
